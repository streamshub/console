...
${KUBE} create serviceaccount dex --dry-run=client -o yaml \
  | ${KUBE} apply -n ${NAMESPACE} -f -
${KUBE} annotate serviceaccount dex -n ${NAMESPACE} --dry-run=client "serviceaccounts.openshift.io/oauth-redirecturi.dex=https://console-dex.${CLUSTER_DOMAIN}/callback" -o yaml \
  | ${KUBE} apply -n ${NAMESPACE} -f -
...
if [ "$(${KUBE} api-resources --api-group=route.openshift.io -o=name)" != "" ] ; then
    ${KUBE} patch ingress/console-dex-ingress -n ${NAMESPACE} --type=merge --patch '{"spec":{"ingressClassName":"openshift-default"}}'
fi