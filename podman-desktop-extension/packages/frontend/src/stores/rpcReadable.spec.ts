...
import { streamshubClient, rpcBrowser } from '/@/api/client';
...
test('check updater is called once at subscription', async () => {
  const spyOnListHistoryInfo = vi.spyOn(streamshubClient, 'listHistoryInfo');
  const rpcWritable = RPCReadable<string[]>([], [], () => {
    streamshubClient.listHistoryInfo();
