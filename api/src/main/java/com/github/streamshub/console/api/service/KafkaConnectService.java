...
public class KafkaConnectService {
    ...
    public CompletionStage<List<ConnectCluster>> listClusters(FieldFilter fields, ListRequestContext<ConnectCluster> listSupport) {
        var pendingServerInfo = consoleConfig.getKafkaConnectClusters()
                ...
                .map(clusterName -> describeCluster(clusterName, fields))
                .toList();
    ...
    public CompletionStage<ConnectCluster> describeCluster(String clusterName, FieldFilter fields) {
        ...
        var pendingPluginResults = includePlugins
        ...
        return client.getWorkerDetails()
                .thenApply(server -> {
                    var cluster = new ConnectCluster(clusterName);
                ...
                .thenCombine(
                    pendingPluginResults,
                    ConnectCluster::plugins);
    }
    ...
    public CompletionStage<Connector> describeConnector(
        ...
        String[] idParts = connectorId.split(";");
        return describeConnector(idParts[0], idParts[1], fields, fetchParams);
    }

    private CompletionStage<List<CompletionStage<Connector>>> listConnectors(
            String clusterName,
            FieldFilter fields,
            FetchParams fetchParams) {

        return kafkaConnectClients.get(clusterName).getConnectors()
                .thenApply(names -> names.parallelStream()
                        .filter(permissionService.permitted(Connector.API_TYPE, Privilege.LIST, Function.identity()))
                        .map(name -> describeConnector(clusterName, name, fields, fetchParams)).toList());
    }
    ...
    private CompletionStage<Connector> describeConnector(
        ...
        var clusterPromise = includeCluster ? describeCluster(clusterName, fields) : PROMISE_NULL_CLUSTER;
        ...
        return client.getConnector(connectorName)
            .thenCombine(
                ...
                (info, state) -> {
                    Connector connector = new Connector(clusterName + ";" + connectorName);
    ...
    private void mapTasks(Connector connector, KafkaConnectAPI.ConnectorInfo info, KafkaConnectAPI.ConnectorStateInfo state) {
        ...
        for (var taskInfo : info.tasks()) {
            var taskState = state.tasks().stream()
                    ...
                    .findFirst();
            var taskId = connector.getId() + ";" + taskInfo.task();
            ...
            task.connector(connector, false);
            tasks.add(task);
        }
