...
import type { Group, ConfigValue } from '../api/groups';
import { pageStyles, tableStyles } from '../styles/shared-styles';

@customElement('group-configuration-page')
export class GroupConfigurationPage extends LitElement {
  ...
  private groupTask = new Task(this, {
    task: async ([kafkaId, groupId]) => {
      const response = await getGroup(
        ...
        groupId as string,
        {
          fields: ['groupId', 'type', 'protocol', 'state', 'configs']
        }
      );
      ...
      <div class="page-content">
        ${this.groupTask.render({
          ...
          complete: (group: Group) => {
            if (!group.attributes.configs) {
              return html`
                <div class="empty-state">
                  <p>No configuration available for this consumer group</p>
                </div>
              `;
            }
            return this.renderConfigTable(group.attributes.configs);
          },
