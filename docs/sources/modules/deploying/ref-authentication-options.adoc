...
<13> Resource name filters that identify which resources the assigned role can access.
You can specify exact names (for example, `dev-cluster-a`), wildcard patterns (for example, `com.example.team.*`), or regular expressions.
When a value is enclosed in slashes (/), the console switches to regular-expression mode (for example, `/qa-cluster-[xy]/`).
